...
ConVar	sk_BotAlly_s_health( "sk_BotAlly_s_health","130");
ConVar	sk_BotAlly_s_kick( "sk_BotAlly_s_kick","15");

ConVar sk_BotAlly_guard_health( "sk_BotAlly_guard_health", "0");
ConVar sk_BotAlly_guard_kick( "sk_BotAlly_guard_kick", "0");
 
// Whether or not the combine guard should spawn health on death
ConVar sk_BotAlly_die_spawn_health( "sk_BotAlly_die_spawn_health", "1" );

extern ConVar sk_plr_dmg_buckshot;	
extern ConVar sk_plr_num_shotgun_pellets;

//Whether or not the combine should spawn health on death
ConVar	BotAlly_spawn_health( "BotAlly_spawn_health", "1" );

LINK_ENTITY_TO_CLASS(bot_ally, CNPC_BotAlly);
...
void CNPC_BotAlly::Spawn( void )
{
	...
	{
		// Stronger, tougher.
		SetHealth( sk_BotAlly_guard_health.GetFloat() );
		SetMaxHealth( sk_BotAlly_guard_health.GetFloat() );
		SetKickDamage( sk_BotAlly_guard_kick.GetFloat() );
	}
	else
	{
		SetHealth( sk_BotAlly_s_health.GetFloat() );
		SetMaxHealth( sk_BotAlly_s_health.GetFloat() );
		SetKickDamage( sk_BotAlly_s_kick.GetFloat() );
	}
...
void CNPC_BotAlly::Precache()
{
	...
	if( !GetModelName() )
	{
		SetModelName( MAKE_STRING( "models/dm/Combine_Soldier.mdl" ) );
	}
...
void CNPC_BotAlly::DeathSound( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
void CNPC_BotAlly::ClearAttackConditions( )
{
...
void CNPC_BotAlly::PrescheduleThink( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_BotAlly::BuildScheduleTestBits( void )
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_BotAlly::SelectSchedule ( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_BotAlly::GetHitgroupDamageMultiplier( int iHitGroup, const CTakeDamageInfo &info )
{
	switch( iHitGroup )
	{
	case HITGROUP_HEAD:
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_BotAlly::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
...
void CNPC_BotAlly::OnChangeActivity( Activity eNewActivity )
{
...
//-----------------------------------------------------------------------------
void CNPC_BotAlly::Event_Killed( const CTakeDamageInfo &info )
{
	// Don't bother if we've been told not to, or the player has a megaphyscannon
	if ( BotAlly_spawn_health.GetBool() == false || PlayerHasMegaPhysCannon() )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_BotAlly::IsLightDamage( const CTakeDamageInfo &info )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_BotAlly::IsHeavyDamage( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
Activity CNPC_BotAlly::NPC_TranslateActivity( Activity eNewActivity )
{
