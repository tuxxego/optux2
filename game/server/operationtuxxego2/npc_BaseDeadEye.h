...
#ifndef NPC_BASEDEADEYE_H
#define NPC_BASEDEADEYE_H
#ifdef _WIN32
#pragma once
#endif
...
#define DEADEYE_ATTN_FOOTSTEP ATTN_IDLE

#define	ENVELOPE_CONTROLLER		(CSoundEnvelopeController::GetController())

#define DEADEYE_MELEE_REACH	55

extern int AE_DEADEYE_ATTACK_RIGHT;
extern int AE_DEADEYE_ATTACK_LEFT;
extern int AE_DEADEYE_ATTACK_BOTH;
extern int AE_DEADEYE_SWATITEM;
extern int AE_DEADEYE_STARTSWAT;
extern int AE_DEADEYE_STEP_LEFT;
extern int AE_DEADEYE_STEP_RIGHT;
extern int AE_DEADEYE_SCUFF_LEFT;
extern int AE_DEADEYE_SCUFF_RIGHT;
extern int AE_DEADEYE_ATTACK_SCREAM;
extern int AE_DEADEYE_GET_UP;
extern int AE_DEADEYE_POUND;

#define DEADEYE_BODYGROUP_HEADCRAB	1	// The crab on our head

// Pass these to claw attack so we know where to draw the blood.
#define DEADEYE_BLOOD_LEFT_HAND		0
#define DEADEYE_BLOOD_RIGHT_HAND		1
#define DEADEYE_BLOOD_BOTH_HANDS		2
#define DEADEYE_BLOOD_BITE			3

#ifdef MAPBASE
#define SF_DEADEYE_NO_TORSO ( 1 << 15 )
#endif
...
enum
{
	SCHED_DEADEYE_CHASE_ENEMY = LAST_SHARED_SCHEDULE,
...
enum
{
	TASK_DEADEYE_DELAY_SWAT = LAST_SHARED_TASK,
	TASK_DEADEYE_GET_PATH_TO_PHYSOBJ,
	TASK_DEADEYE_SWAT_ITEM,
	TASK_DEADEYE_DIE,
	TASK_DEADEYE_RELEASE_HEADCRAB,
	TASK_DEADEYE_WAIT_POST_MELEE,

	LAST_BASE_DEADEYE_TASK,
};
...
enum DeadEye_Conds
{
	COND_DEADEYE_CAN_SWAT_ATTACK = LAST_SHARED_CONDITION,
	COND_DEADEYE_RELEASECRAB,
	COND_DEADEYE_LOCAL_MELEE_OBSTRUCTION,

	LAST_BASE_DEADEYE_CONDITION,
};



typedef CAI_BlendingHost< CAI_BehaviorHost<CAI_BaseNPC> > CAI_BaseDeadEyeBase;

//=========================================================
//=========================================================
abstract_class CNPC_BaseDeadEye : public CAI_BaseDeadEyeBase
{
	DECLARE_CLASS(CNPC_BaseDeadEye, CAI_BaseDeadEyeBase);

public:
	CNPC_BaseDeadEye(void);
	~CNPC_BaseDeadEye(void);
	...
	virtual float GetClawAttackRange() const { return DEADEYE_MELEE_REACH; }
	...
	bool ShouldIgniteDeadEyeGib(void);
	...
	virtual void SetDeadEyeModel(void) { };
...
protected:
	...
	bool	m_fIsTorso;			// is this is a half-deadeye?
	bool	m_fIsHeadless;		// is this deadeye headless
	...
	//
	// DeadEyes catch on fire if they take too much burn damage in a given time period.
	//
	...
	static int g_numDeadEyes;	// counts total number of existing deadeyes.

	int m_iMoanSound; // each deadeye picks one of the 4 and keeps it.

	static int ACT_DEADEYE_SWATLEFTMID;
	static int ACT_DEADEYE_SWATRIGHTMID;
	static int ACT_DEADEYE_SWATLEFTLOW;
	static int ACT_DEADEYE_SWATRIGHTLOW;
	static int ACT_DEADEYE_RELEASECRAB;
	static int ACT_DEADEYE_FALL;
