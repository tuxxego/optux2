...
//
// Purpose: Implements the deadeye, a horrific once-human headcrab victim.
//
// The deadeye has two main states: Full and Torso.
//
// In Full state, the deadeye is whole and walks upright as he did in Half-Life.
// He will try to claw the player and swat physics items at him. 
//
// In Torso state, the deadeye has been blasted or cut in half, and the Torso will
// drag itself along the ground with its arms. It will try to claw the player.
//
// In either state, a severely injured DeadEye will release its headcrab, which
// will immediately go after the player. The DeadEye will then die (ragdoll). 
//
//=============================================================================//
...
#define DEADEYE_BULLET_DAMAGE_SCALE 0.5f

int g_interactionDeadEyeMeleeWarning;

envelopePoint_t envDefaultDeadEyeMoanVolumeFast[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envDefaultDeadEyeMoanVolume[] =
{
	{ 1.0f, 0.1f,
	0.1f, 0.1f,
	},
...
// if the deadeye doesn't find anything closer than this, it doesn't swat.
#define DEADEYE_FARTHEST_PHYSICS_OBJECT	40.0*12.0
#define DEADEYE_PHYSICS_SEARCH_DEPTH	100

// Don't swat objects unless player is closer than this.
#define DEADEYE_PLAYER_MAX_SWAT_DIST		1000

//
// How much health a DeadEye torso gets when a whole deadeye is broken
// It's whole deadeye's MAX Health * this value
#define DEADEYE_TORSO_HEALTH_FACTOR 0.5

//
// When the deadeye has health < m_iMaxHealth * this value, it will
// try to release its headcrab.
#define DEADEYE_RELEASE_HEALTH_FACTOR	0.5

//
// The heaviest physics object that a deadeye should try to swat. (kg)
#define DEADEYE_MAX_PHYSOBJ_MASS		60

//
// DeadEye tries to get this close to a physics object's origin to swat it
#define DEADEYE_PHYSOBJ_SWATDIST		80

//
// Because movement code sometimes doesn't get us QUITE where we
// want to go, the deadeye tries to get this close to a physics object
// DeadEye will end up somewhere between PHYSOBJ_MOVE_TO_DIST & PHYSOBJ_SWATDIST
#define DEADEYE_PHYSOBJ_MOVE_TO_DIST	48

//
// How long between physics swat attacks (in seconds). 
#define DEADEYE_SWAT_DELAY			5


//
// After taking damage, ignore further damage for n seconds. This keeps the deadeye
// from being interrupted while.
//
#define DEADEYE_FLINCH_DELAY			3


#define DEADEYE_BURN_TIME		10 // If ignited, burn for this many seconds
#define DEADEYE_BURN_TIME_NOISE	2  // Give or take this many seconds.


//=========================================================
// private activities
//=========================================================
int CNPC_BaseDeadEye::ACT_DEADEYE_SWATLEFTMID;
int CNPC_BaseDeadEye::ACT_DEADEYE_SWATRIGHTMID;
int CNPC_BaseDeadEye::ACT_DEADEYE_SWATLEFTLOW;
int CNPC_BaseDeadEye::ACT_DEADEYE_SWATRIGHTLOW;
int CNPC_BaseDeadEye::ACT_DEADEYE_RELEASECRAB;
int CNPC_BaseDeadEye::ACT_DEADEYE_FALL;

ConVar	sk_deadeye_dmg_one_slash("sk_deadeye_dmg_one_slash", "0");
ConVar	sk_deadeye_dmg_both_slash("sk_deadeye_dmg_both_slash", "0");


// When a deadeye spawns, he will select a 'base' pitch value
// that's somewhere between basepitchmin & basepitchmax
ConVar deadeye_basemin("deadeye_basemin", "100");
ConVar deadeye_basemax("deadeye_basemax", "100");

ConVar deadeye_changemin("deadeye_changemin", "0");
ConVar deadeye_changemax("deadeye_changemax", "0");

// play a sound once in every deadeye_stepfreq steps
ConVar deadeye_stepfreq("deadeye_stepfreq", "4");
ConVar deadeye_moanfreq("deadeye_moanfreq", "1");

ConVar deadeye_decaymin("deadeye_decaymin", "0.1");
ConVar deadeye_decaymax("deadeye_decaymax", "0.4");

ConVar deadeye_ambushdist("deadeye_ambushdist", "16000");

//=========================================================
// For a couple of reasons, we keep a running count of how
// many deadeyes in the world are angry at any given time.
//=========================================================
static int s_iAngryDeadEyes = 0;

//=========================================================
//=========================================================
class CAngryDeadEyeCounter : public CAutoGameSystem
{
public:
	CAngryDeadEyeCounter(char const *name) : CAutoGameSystem(name)
	{
	}
	// Level init, shutdown
...
CAngryDeadEyeCounter	AngryDeadEyeCounter("CAngryDeadEyeCounter");


int AE_DEADEYE_ATTACK_RIGHT;
int AE_DEADEYE_ATTACK_LEFT;
int AE_DEADEYE_ATTACK_BOTH;
int AE_DEADEYE_SWATITEM;
int AE_DEADEYE_STARTSWAT;
int AE_DEADEYE_STEP_LEFT;
int AE_DEADEYE_STEP_RIGHT;
int AE_DEADEYE_SCUFF_LEFT;
int AE_DEADEYE_SCUFF_RIGHT;
int AE_DEADEYE_ATTACK_SCREAM;
int AE_DEADEYE_GET_UP;
int AE_DEADEYE_POUND;
int AE_DEADEYE_ALERTSOUND;
int AE_DEADEYE_POPHEADCRAB;


//=========================================================
//=========================================================
BEGIN_DATADESC(CNPC_BaseDeadEye)
...
//LINK_ENTITY_TO_CLASS( base_deadeye, CNPC_BaseDeadEye );

//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_BaseDeadEye::g_numDeadEyes = 0;


//---------------------------------------------------------
//---------------------------------------------------------
CNPC_BaseDeadEye::CNPC_BaseDeadEye()
{
	...
	// This means if 3 or so deadeyes spawn near each
	// other, they will definitely not pick the same
	// moan loop.
	m_iMoanSound = g_numDeadEyes;
...
// <= MaxMass in Mass
// Between the deadeye and the enemy
// not too far from a direct line to the enemy.
//---------------------------------------------------------
bool CNPC_BaseDeadEye::FindNearestPhysicsObject(int iMaxMass)
{
	CBaseEntity		*pList[DEADEYE_PHYSICS_SEARCH_DEPTH];
	...
	if (dist > DEADEYE_PLAYER_MAX_SWAT_DIST)
	{
	...
	float flNearestDist = MIN(dist, DEADEYE_FARTHEST_PHYSICS_OBJECT * 0.5);
	Vector vecDelta(flNearestDist, flNearestDist, GetHullHeight() * 2.0);

	class CDeadEyeSwatEntitiesEnum : public CFlaggedEntitiesEnum
	{
	public:
		CDeadEyeSwatEntitiesEnum(CBaseEntity **pList, int listMax, int iMaxMass)
		...
		virtual IterationRetval_t EnumElement(IHandleEntity *pHandleEntity)
		{
				...
				pEntity->VPhysicsGetObject()->IsMoveable())
			{
				...
				//if (pEntity->HasSpawnFlags(SF_PHYSPROP_NO_DEADEYE_SWAT) && dynamic_cast<CPhysicsProp*>(pEntity))
	...
	CDeadEyeSwatEntitiesEnum swatEnum(pList, DEADEYE_PHYSICS_SEARCH_DEPTH, iMaxMass);

	int count = UTIL_EntitiesInBox(GetAbsOrigin() - vecDelta, GetAbsOrigin() + vecDelta, &swatEnum);

	// magically know where they are
	Vector vecDeadEyeKnees;
	CollisionProp()->NormalizedToWorldSpace(Vector(0.5f, 0.5f, 0.25f), &vecDeadEyeKnees);
	...
	for (i = 0; i < count; i++)
	{
		...
		// This object is closer... but is it between the player and the deadeye?
		...
		if ((center.z + pList[i]->BoundingRadius()) < vecDeadEyeKnees.z)
			continue;
		...
		if (objMaxs.z < vecDeadEyeKnees.z)
			continue;
		...
		if (hl2_episodic.GetBool())
		{
			...
			// The case for this feature is that deadeyes who are pursuing the player will
			...
			// makes the deadeyes very late (in a random fashion) getting where they are going. (sjb 1/2/06)
		...
		// The object must also be closer to the deadeye than it is to the enemy
		pNearest = pList[i];
...
//-----------------------------------------------------------------------------
Class_T	CNPC_BaseDeadEye::Classify(void)
{
	if (IsSlumped())
		return CLASS_NONE;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Disposition_t CNPC_BaseDeadEye::IRelationType(CBaseEntity *pTarget)
{
	// Slumping should not affect DeadEye's opinion of others
	if (IsSlumped())
	{
		m_bIsSlumped = false;
...
//-----------------------------------------------------------------------------
float CNPC_BaseDeadEye::MaxYawSpeed(void)
{
	if (m_fIsTorso)
	{
		return(60);
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_BaseDeadEye::OverrideMoveFacing(const AILocalMoveGoal_t &move, float flInterval)
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_BaseDeadEye::MeleeAttack1Conditions(float flDot, float flDist)
{
	...
	if (tr.fraction == 1.0 || !tr.m_pEnt)
	{
		...
		// This attack would miss completely. Trick the deadeye into moving around some more.
		return COND_TOO_FAR_TO_ATTACK;
	}
	...
	{
		// -Let the deadeye swipe at his enemy if he's going to hit them.
		// -Also let him swipe at NPC's that happen to be between the deadeye and the enemy. 
		//  This makes mobs of deadeyes seem more rowdy since it doesn't leave guys in the back row standing around.
	...
	{
		// The trace hit something solid, but it's not the enemy. If this item is closer to the deadeye than
		...
		{
			return COND_DEADEYE_LOCAL_MELEE_OBSTRUCTION;
		}
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#define DEADEYE_BUCKSHOT_TRIPLE_DAMAGE_DIST	96.0f // Triple damage from buckshot at 8 feet (headshot only)
float CNPC_BaseDeadEye::GetHitgroupDamageMultiplier(int iHitGroup, const CTakeDamageInfo &info)
{
	switch (iHitGroup)
	{
	case HITGROUP_HEAD:
	{
		if (info.GetDamageType() & DMG_BUCKSHOT)
		{
			...
			if (flDist <= DEADEYE_BUCKSHOT_TRIPLE_DAMAGE_DIST)
			{
				return 3.0f;
			}
		}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
	...
	{
		// DeadEye gets across-the-board damage reduction for buckshot. This compensates for the recent changes which
		// make the shotgun much more powerful, and returns the deadeyes to a level that has been playtested extensively.(sjb)
...
//-----------------------------------------------------------------------------
// Purpose: A deadeye has taken damage. Determine whether he should split in half
// Input  : 
// Output : bool, true if yes.
//-----------------------------------------------------------------------------
bool CNPC_BaseDeadEye::ShouldBecomeTorso(const CTakeDamageInfo &info, float flDamageThreshold)
{
	if (info.GetDamageType() & DMG_REMOVENORAGDOLL)
		return false;

#ifdef MAPBASE
	if (HasSpawnFlags(SF_DEADEYE_NO_TORSO))
		return false;
#endif
...
//-----------------------------------------------------------------------------
// Purpose: A deadeye has taken damage. Determine whether he release his headcrab.
// Output : YES, IMMEDIATE, or SCHEDULED (see HeadcrabRelease_t)
//-----------------------------------------------------------------------------
HeadcrabRelease_t CNPC_BaseDeadEye::ShouldReleaseHeadcrab(const CTakeDamageInfo &info, float flDamageThreshold)
{
...
// Output : int
//-----------------------------------------------------------------------------
#define DEADEYE_SCORCH_RATE		8
#define DEADEYE_MIN_RENDERCOLOR	50
int CNPC_BaseDeadEye::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
	CTakeDamageInfo info = inputInfo;

	if (inputInfo.GetDamageType() & DMG_BURN)
	{
		// If a deadeye is on fire it only takes damage from the fire that's attached to it. (DMG_DIRECT)
		// This is to stop deadeyes from burning to death 10x faster when they're standing around
		// 10 fire entities.
		...
		Scorch(DEADEYE_SCORCH_RATE, DEADEYE_MIN_RENDERCOLOR);
	}
	...
	{
		info.ScaleDamage(DEADEYE_BULLET_DAMAGE_SCALE);
	}
...
#ifdef MAPBASE
	if (!m_fIsTorso && (bChopped || bSquashed) && !bKilledByVehicle && !(info.GetDamageType() & DMG_REMOVENORAGDOLL) && !HasSpawnFlags(SF_DEADEYE_NO_TORSO))
#else
	...
	else
	{
		...
		case RELEASE_SCHEDULED:
			SetCondition(COND_DEADEYE_RELEASECRAB);
			break;
		}
	...
	{
		//!!!HACKHACK- Stuff a light_damage condition if an actbusying deadeye takes direct burn damage. This will cause an
		// ignited deadeye to 'wake up' and rise out of its actbusy slump. (sjb)
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::MakeAISpookySound(float volume, float duration)
{
#ifdef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_BaseDeadEye::CanPlayMoanSound()
{
	if (HasSpawnFlags(SF_NPC_GAG))
		return false;

	// Burning deadeyes play their moan loop at full volume for as long as they're
	// burning. Don't let a moan envelope play cause it will turn the volume down when done.
	if (IsOnFire())
		return false;

	// Members of a small group of deadeyes can vocalize whenever they want
	if (s_iAngryDeadEyes <= 4)
		return true;

	// This serves to limit the number of deadeyes that can moan at one time when there are a lot. 
	if (random->RandomInt(1, deadeye_moanfreq.GetInt() * (s_iAngryDeadEyes / 2)) == 1)
	{
		return true;
	}
...
//			come through.
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::MoanSound(envelopePoint_t *pEnvelope, int iEnvelopeSize)
{
	if (HasSpawnFlags(SF_NPC_GAG))
	{
		// Not yet!
	...
	if (!m_pMoanSound)
	{
		...
		m_flMoanPitch = random->RandomInt(deadeye_basemin.GetInt(), deadeye_basemax.GetInt());
	...
	envDefaultDeadEyeMoanVolumeFast[1].durationMin = deadeye_decaymin.GetFloat();
	envDefaultDeadEyeMoanVolumeFast[1].durationMax = deadeye_decaymax.GetFloat();
	...
	float flPitch = random->RandomInt(m_flMoanPitch + deadeye_changemin.GetInt(), m_flMoanPitch + deadeye_changemax.GetInt());
...
//-----------------------------------------------------------------------------
// Purpose: Determine whether the deadeye is chopped up by some physics item
//-----------------------------------------------------------------------------
bool CNPC_BaseDeadEye::IsChopped(const CTakeDamageInfo &info)
{
	float flDamageThreshold = MIN(1, info.GetDamage() / m_iMaxHealth);
...
//-----------------------------------------------------------------------------
// Purpose: Return true if this gibbing deadeye should ignite its gibs
//-----------------------------------------------------------------------------
bool CNPC_BaseDeadEye::ShouldIgniteDeadEyeGib(void)
{
#ifdef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
// Purpose: Handle the special case of a deadeye killed by a physics chopper.
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::DieChopped(const CTakeDamageInfo &info)
{
	bool bSquashed = IsSquashed(info);
...
#ifdef MAPBASE
	// Hack for fast deadeyes using base torso rules.
	if (m_iHealth > 0)
	{
		SetHealth(0);
	}
	...
	if (m_bForceServerRagdoll)
	{
		...
		pLegGib = CreateRagGib(GetLegsModel(), GetAbsOrigin(), GetAbsAngles(), vecLegsForce, flFadeTime, ShouldIgniteDeadEyeGib());
#else
	CBaseEntity *pLegGib = CreateRagGib(GetLegsModel(), GetAbsOrigin(), GetAbsAngles(), vecLegsForce, flFadeTime, ShouldIgniteDeadEyeGib());
#endif
	if (pLegGib)
	{
#ifdef MAPBASE
	...
	if (m_bForceServerRagdoll)
	{
		...
		pTorsoGib = CreateRagGib(GetTorsoModel(), GetAbsOrigin() + Vector(0, 0, 64), TorsoAngles, forceVector, flFadeTime, ShouldIgniteDeadEyeGib());
#else
	CBaseEntity *pTorsoGib = CreateRagGib(GetTorsoModel(), GetAbsOrigin() + Vector(0, 0, 64), TorsoAngles, forceVector, flFadeTime, ShouldIgniteDeadEyeGib());
#endif
	if (pTorsoGib)
	{
		...
		if (pAnimating)
		{
			pAnimating->SetBodygroup(DEADEYE_BODYGROUP_HEADCRAB, !m_fIsHeadless);
		}
...
//-----------------------------------------------------------------------------
// Purpose: damage has been done. Should the deadeye ignite?
//-----------------------------------------------------------------------------
bool CNPC_BaseDeadEye::ShouldIgnite(const CTakeDamageInfo &info)
{
	if (IsOnFire())
	{
		// Already burning!
...
//-----------------------------------------------------------------------------
// Purpose: Sufficient fire damage has been done. DeadEye ignites!
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner)
{
	...
	// Set the deadeye up to burn to death in about ten seconds.
	SetHealth(MIN(m_iHealth, FLAME_DIRECT_DAMAGE_PER_SEC * (DEADEYE_BURN_TIME + random->RandomFloat(-DEADEYE_BURN_TIME_NOISE, DEADEYE_BURN_TIME_NOISE))));

	// FIXME: use overlays when they come online
	//AddOverlay( ACT_DEADEYE_WALK_ON_FIRE, false );
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_BaseDeadEye::CopyRenderColorTo(CBaseEntity *pOther)
{
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_BaseDeadEye::ClawAttack(float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch, int BloodOrigin)
{
	...
	if (!pHurt && m_hPhysicsEnt != NULL && IsCurSchedule(SCHED_DEADEYE_ATTACKITEM))
	{
		pHurt = m_hPhysicsEnt;
	...
	if (pHurt == m_hPhysicsEnt && IsCurSchedule(SCHED_DEADEYE_ATTACKITEM))
	{
		m_hPhysicsEnt = NULL;
...
//-----------------------------------------------------------------------------
// Purpose: The deadeye is frustrated and pounding walls/doors. Make an appropriate noise
// Input  : 
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::PoundSound()
{
	trace_t		tr;
	Vector		forward;
	...
	CPASAttenuationFilter filter(this, "NPC_BaseDeadEye.PoundDoor");
	EmitSound(filter, entindex(), "NPC_BaseDeadEye.PoundDoor");
}
...
// Input  : pEvent - 
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::HandleAnimEvent(animevent_t *pEvent)
{
	if (pEvent->event == AE_NPC_ATTACK_BROADCAST)
	{
		if (GetEnemy() && GetEnemy()->IsNPC())
		{
			if (HasCondition(COND_CAN_MELEE_ATTACK1))
			{
				...
				// (For instance, code that makes a deadeye take a frustrated swipe at an obstacle). 
				// Try not to trigger a reaction from our enemy unless we're really attacking. 
				GetEnemy()->MyNPCPointer()->DispatchInteraction(g_interactionDeadEyeMeleeWarning, NULL, this);
			}
		}
		return;
	}

	if (pEvent->event == AE_DEADEYE_POUND)
	{
		PoundSound();
		return;
	}

	if (pEvent->event == AE_DEADEYE_ALERTSOUND)
	{
		AlertSound();
		return;
	}

	if (pEvent->event == AE_DEADEYE_STEP_LEFT)
	{
	...
	if (pEvent->event == AE_DEADEYE_STEP_RIGHT)
	{
		MakeAIFootstepSound(180.0f);
		FootstepSound(true);
		return;
	...
	if (pEvent->event == AE_DEADEYE_GET_UP)
	{
		MakeAIFootstepSound(180.0f, 3.0f);
		if (!IsOnFire())
		{
			// If you let this code run while a deadeye is burning, it will stop wailing. 
			m_flNextMoanSound = gpGlobals->curtime;
			MoanSound(envDefaultDeadEyeMoanVolumeFast, ARRAYSIZE(envDefaultDeadEyeMoanVolumeFast));
		}
		return;
	}

	if (pEvent->event == AE_DEADEYE_SCUFF_LEFT)
	{
		MakeAIFootstepSound(180.0f);
	...
	if (pEvent->event == AE_DEADEYE_SCUFF_RIGHT)
	{
		MakeAIFootstepSound(180.0f);
		FootscuffSound(true);
		return;
	...
	if (pEvent->event == AE_DEADEYE_STARTSWAT)
	{
		MakeAIFootstepSound(180.0f);
		AttackSound();
		return;
	...
	if (pEvent->event == AE_DEADEYE_ATTACK_SCREAM)
	{
		AttackSound();
		return;
	}

	if (pEvent->event == AE_DEADEYE_SWATITEM)
	{
		...
		if (pEnemy)
		{
			...
			if (!pPhysicsEntity)
			{
				DevMsg("**DeadEye: Missing my physics ent!!");
				return;
			}
			...
			if (!pPhysObj)
			{
				DevMsg("**DeadEye: No Physics Object for physics Ent!");
				return;
			}

			EmitSound("NPC_BaseDeadEye.Swat");
			...
			// If we don't put the object scan time well into the future, the deadeye
			// will re-select the object he just hit as it is flying away from him.
			// It will likely always be the nearest object because the deadeye moved
			// close enough to it to hit it.
			m_hPhysicsEnt = NULL;

			m_flNextSwatScan = gpGlobals->curtime + DEADEYE_SWAT_DELAY;

			return;
		}
	}

	if (pEvent->event == AE_DEADEYE_ATTACK_RIGHT)
	{
		...
		ClawAttack(GetClawAttackRange(), sk_deadeye_dmg_one_slash.GetFloat(), qa, vec, DEADEYE_BLOOD_RIGHT_HAND);
		return;
	}

	if (pEvent->event == AE_DEADEYE_ATTACK_LEFT)
	{
		Vector right, forward;
		AngleVectors(GetLocalAngles(), &forward, &right, NULL);
		...
		QAngle qa(-15, 20, -10);
		Vector vec = right + forward;
		ClawAttack(GetClawAttackRange(), sk_deadeye_dmg_one_slash.GetFloat(), qa, vec, DEADEYE_BLOOD_LEFT_HAND);
		return;
	}

	if (pEvent->event == AE_DEADEYE_ATTACK_BOTH)
	{
		Vector forward;
		QAngle qaPunch(45, random->RandomInt(-5, 5), random->RandomInt(-5, 5));
		AngleVectors(GetLocalAngles(), &forward);
		forward = forward * 200;
		ClawAttack(GetClawAttackRange(), sk_deadeye_dmg_one_slash.GetFloat(), qaPunch, forward, DEADEYE_BLOOD_BOTH_HANDS);
		return;
	}

	if (pEvent->event == AE_DEADEYE_POPHEADCRAB)
	{
		...
		if (boneIndex == -1)
		{
			Warning("AE_DEADEYE_POPHEADCRAB event using invalid bone name! Usage: event AE_DEADEYE_POPHEADCRAB \"<BoneName> <Speed>\" \n");
			return;
		}
		...
		if (!token)
		{
			Warning("AE_DEADEYE_POPHEADCRAB event format missing velocity parameter! Usage: event AE_DEADEYE_POPHEADCRAB \"<BoneName> <Speed>\" \n");
			return;
		}
...
//-----------------------------------------------------------------------------
// Purpose: Spawn function for the base deadeye.
//
...
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::Spawn(void)
{
	SetSolid(SOLID_BBOX);
	...
	// DeadEyes get to cheat for 6 seconds (sjb)
...
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::Precache(void)
{
	UTIL_PrecacheOther(GetHeadcrabClassname());

	PrecacheScriptSound("E3_Phystown.Slicer");
	PrecacheScriptSound("NPC_BaseDeadEye.PoundDoor");
	PrecacheScriptSound("NPC_BaseDeadEye.Swat");

	PrecacheModel(GetLegsModel());
	PrecacheModel(GetTorsoModel());

	PrecacheParticleSystem("blood_impact_deadeye_01");
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_BaseDeadEye::StartTouch(CBaseEntity *pOther)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_BaseDeadEye::CreateBehaviors()
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_BaseDeadEye::TranslateSchedule(int scheduleType)
{
	switch (scheduleType)
	{
	case SCHED_CHASE_ENEMY:
		if (HasCondition(COND_DEADEYE_LOCAL_MELEE_OBSTRUCTION) && !HasCondition(COND_TASK_FAILED) && IsCurSchedule(SCHED_DEADEYE_CHASE_ENEMY, false))
		{
			return SCHED_COMBAT_PATROL;
		}
		return SCHED_DEADEYE_CHASE_ENEMY;
		break;

	case SCHED_DEADEYE_SWATITEM:
		// If the object is far away, move and swat it. If it's close, just swat it.
		if (DistToPhysicsEnt() > DEADEYE_PHYSOBJ_SWATDIST)
		{
			return SCHED_DEADEYE_MOVE_SWATITEM;
		}
		else
		{
			return SCHED_DEADEYE_SWATITEM;
		}
		break;

	case SCHED_STANDOFF:
		return SCHED_DEADEYE_WANDER_STANDOFF;

	case SCHED_MELEE_ATTACK1:
		return SCHED_DEADEYE_MELEE_ATTACK1;
	}
...
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::BuildScheduleTestBits(void)
{
	...
	SetCustomInterruptCondition(COND_DEADEYE_RELEASECRAB);
...
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::OnScheduleChange(void)
{
	//
	...
	{
		m_flNextFlinch = gpGlobals->curtime + DEADEYE_FLINCH_DELAY;
	}
...
//---------------------------------------------------------
//---------------------------------------------------------
int	CNPC_BaseDeadEye::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
	if (failedSchedule == SCHED_DEADEYE_WANDER_MEDIUM)
	{
		return SCHED_DEADEYE_WANDER_FAIL;
	}
	...
	if (CanSwatPhysicsObjects())
	{
			...
			m_hObstructor->VPhysicsGetObject()->GetMass() < 100)
		{
			...
			m_hObstructor = NULL;
			return SCHED_DEADEYE_ATTACKITEM;
		}
	}
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_BaseDeadEye::SelectSchedule(void)
{
	if (HasCondition(COND_DEADEYE_RELEASECRAB))
	{
		// Death waits for no man. Or deadeye. Or something.
		return SCHED_DEADEYE_RELEASECRAB;
	}
	...
	case NPC_STATE_COMBAT:
		if (HasCondition(COND_NEW_ENEMY) && GetEnemy())
		{
			...
			if (flDist >= deadeye_ambushdist.GetFloat() && MustCloseToAttack())
			{
				return SCHED_DEADEYE_MOVE_TO_AMBUSH;
			}
		}

		if (HasCondition(COND_LOST_ENEMY) || (HasCondition(COND_ENEMY_UNREACHABLE) && MustCloseToAttack()))
		{
			return SCHED_DEADEYE_WANDER_MEDIUM;
		}

		if (HasCondition(COND_DEADEYE_CAN_SWAT_ATTACK))
		{
			return SCHED_DEADEYE_SWATITEM;
		}
		break;
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_BaseDeadEye::IsSlumped(void)
{
	if (hl2_episodic.GetBool())
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_BaseDeadEye::IsGettingUp(void)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_BaseDeadEye::GetSwatActivity(void)
{
	...
	if (flDot >= 0)
	{
		...
		if (flZDiff < 0)
		{
			return ACT_DEADEYE_SWATRIGHTMID;
		}

		return ACT_DEADEYE_SWATRIGHTLOW;
	}
	else
	{
		// Left
		if (flZDiff < 0)
		{
			return ACT_DEADEYE_SWATLEFTMID;
		}

		return ACT_DEADEYE_SWATLEFTLOW;
	}
}
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_BaseDeadEye::GatherConditions(void)
{
	ClearCondition(COND_DEADEYE_LOCAL_MELEE_OBSTRUCTION);
	...
	if (m_NPCState == NPC_STATE_COMBAT && !m_fIsTorso)
	{
		...
		// eliminates the deadeye picking a better physics object if one happens to fall
		// between him and the object he's heading for already. 
		if (gpGlobals->curtime >= m_flNextSwatScan && (m_hPhysicsEnt == NULL))
		{
			FindNearestPhysicsObject(DEADEYE_MAX_PHYSOBJ_MASS);
			m_flNextSwatScan = gpGlobals->curtime + 2.0;
		}
	}

	if ((m_hPhysicsEnt != NULL) && gpGlobals->curtime >= m_flNextSwat && HasCondition(COND_SEE_ENEMY) && !HasCondition(COND_DEADEYE_RELEASECRAB))
	{
		SetCondition(COND_DEADEYE_CAN_SWAT_ATTACK);
	}
	else
	{
		ClearCondition(COND_DEADEYE_CAN_SWAT_ATTACK);
	}
}

//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_BaseDeadEye::PrescheduleThink(void)
{
	BaseClass::PrescheduleThink();

#if 0
	DevMsg(" ** %d Angry DeadEyes **\n", s_iAngryDeadEyes);
#endif

#if 0
	if (m_NPCState == NPC_STATE_COMBAT)
	{
		// DeadEyes should make idle sounds in combat
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_BaseDeadEye::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_DEADEYE_DIE:
		// Go to ragdoll
		KillMe();
		TaskComplete();
		break;

	case TASK_DEADEYE_GET_PATH_TO_PHYSOBJ:
	{
		Vector vecGoalPos;
		Vector vecDir;
	...
	case TASK_DEADEYE_SWAT_ITEM:
	{
		...
		else if (DistToPhysicsEnt() > DEADEYE_PHYSOBJ_SWATDIST)
		{
			// Physics ent is no longer in range! Probably another deadeye swatted it or it moved
	...
	case TASK_DEADEYE_DELAY_SWAT:
		m_flNextSwat = gpGlobals->curtime + pTask->flTaskData;
		TaskComplete();
		break;

	case TASK_DEADEYE_RELEASE_HEADCRAB:
	{
	...
	case TASK_DEADEYE_WAIT_POST_MELEE:
	{
#ifndef HL2_EPISODIC
		TaskComplete();
		return;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_BaseDeadEye::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_DEADEYE_SWAT_ITEM:
...
//---------------------------------------------------------
// Make the necessary changes to a deadeye to make him a 
// torso!
//---------------------------------------------------------
void CNPC_BaseDeadEye::BecomeTorso(const Vector &vecTorsoForce, const Vector &vecLegsForce)
{
	if (m_fIsTorso)
	{
		DevMsg("*** DeadEye is already a torso!\n");
		return;
	}
	...
	if (!m_fIsHeadless)
	{
		m_iMaxHealth = DEADEYE_TORSO_HEALTH_FACTOR * m_iMaxHealth;
		...
		// Put the deadeye in a TOSS / fall schedule
		// Otherwise he fails and sits on the ground for a sec.
		SetSchedule(SCHED_FALL_TO_GROUND);

		m_fIsTorso = true;

		// Put the torso up where the torso was when the deadeye
		// was whole.
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_BaseDeadEye::Event_Killed(const CTakeDamageInfo &info)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_BaseDeadEye::BecomeRagdoll(const CTakeDamageInfo &info, const Vector &forceVector)
{
	bool bKilledByVehicle = ((info.GetDamageType() & DMG_VEHICLE) != 0);
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_BaseDeadEye::StopLoopingSounds()
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_BaseDeadEye::RemoveHead(void)
{
	m_fIsHeadless = true;
	SetDeadEyeModel();
}
...
bool CNPC_BaseDeadEye::ShouldPlayFootstepMoan(void)
{
	if (random->RandomInt(1, deadeye_stepfreq.GetInt() * s_iAngryDeadEyes) == 1)
	{
		return true;
	}
...
#define DEADEYE_CRAB_INHERITED_SPAWNFLAGS	(SF_NPC_GAG|SF_NPC_LONG_RANGE|SF_NPC_FADE_CORPSE|SF_NPC_ALWAYSTHINK)
#define CRAB_HULL_EXPAND	1.1f
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_BaseDeadEye::HeadcrabFits(CBaseAnimating *pCrab)
{
...
//			fRagdollBody - 
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::ReleaseHeadcrab(const Vector &vecOrigin, const Vector &vecVelocity, bool fRemoveHead, bool fRagdollBody, bool fRagdollCrab)
{
	CAI_BaseNPC		*pCrab;
	...
	if (fRagdollCrab)
	{
		...
		if (m_bForceServerRagdoll)
		{
			...
			pGib = CreateRagGib(GetHeadcrabModel(), vecOrigin, GetLocalAngles(), vecVelocity, 15, ShouldIgniteDeadEyeGib());
#else
		CBaseEntity *pGib = CreateRagGib(GetHeadcrabModel(), vecOrigin, GetLocalAngles(), vecVelocity, 15, ShouldIgniteDeadEyeGib());
#endif
	...
	else
	{
		...
		// Stick the crab in whatever squad the deadeye was in.
		...
		pCrab->AddSpawnFlags(m_spawnflags & DEADEYE_CRAB_INHERITED_SPAWNFLAGS);
...
void CNPC_BaseDeadEye::SetHeadcrabSpawnLocation(int iCrabAttachment, CBaseAnimating *pCrab)
{
...
float CNPC_BaseDeadEye::DistToPhysicsEnt(void)
{
	...
	return DEADEYE_PHYSOBJ_SWATDIST + 1;
}


//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::OnStateChange(NPC_STATE OldState, NPC_STATE NewState)
{
	switch (NewState)
	{
	case NPC_STATE_COMBAT:
...
//-----------------------------------------------------------------------------
Activity CNPC_BaseDeadEye::NPC_TranslateActivity(Activity baseAct)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CNPC_BaseDeadEye::BodyTarget(const Vector &posSrc, bool bNoisy)
{

	if (IsCurSchedule(SCHED_BIG_FLINCH) || m_ActBusyBehavior.IsActive())
	{
		// This deadeye is assumed to be standing up. 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CNPC_BaseDeadEye::HeadTarget(const Vector &posSrc)
{
	int iCrabAttachment = LookupAttachment("headcrab");
	Assert(iCrabAttachment > 0);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_BaseDeadEye::GetAutoAimRadius()
{
	if (m_fIsTorso)
	{
		return 12.0f;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_BaseDeadEye::OnInsufficientStopDist(AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult)
{
...
//			&chasePosition - 
//-----------------------------------------------------------------------------
void CNPC_BaseDeadEye::TranslateNavGoal(CBaseEntity *pEnemy, Vector &chasePosition)
{
...
AI_BEGIN_CUSTOM_NPC(base_deadeye, CNPC_BaseDeadEye)

DECLARE_TASK(TASK_DEADEYE_DELAY_SWAT)
DECLARE_TASK(TASK_DEADEYE_SWAT_ITEM)
DECLARE_TASK(TASK_DEADEYE_GET_PATH_TO_PHYSOBJ)
DECLARE_TASK(TASK_DEADEYE_DIE)
DECLARE_TASK(TASK_DEADEYE_RELEASE_HEADCRAB)
DECLARE_TASK(TASK_DEADEYE_WAIT_POST_MELEE)

DECLARE_ACTIVITY(ACT_DEADEYE_SWATLEFTMID)
DECLARE_ACTIVITY(ACT_DEADEYE_SWATRIGHTMID)
DECLARE_ACTIVITY(ACT_DEADEYE_SWATLEFTLOW)
DECLARE_ACTIVITY(ACT_DEADEYE_SWATRIGHTLOW)
DECLARE_ACTIVITY(ACT_DEADEYE_RELEASECRAB)
DECLARE_ACTIVITY(ACT_DEADEYE_FALL)

DECLARE_CONDITION(COND_DEADEYE_CAN_SWAT_ATTACK)
DECLARE_CONDITION(COND_DEADEYE_RELEASECRAB)
DECLARE_CONDITION(COND_DEADEYE_LOCAL_MELEE_OBSTRUCTION)

//Adrian: events go here
DECLARE_ANIMEVENT(AE_DEADEYE_ATTACK_RIGHT)
DECLARE_ANIMEVENT(AE_DEADEYE_ATTACK_LEFT)
DECLARE_ANIMEVENT(AE_DEADEYE_ATTACK_BOTH)
DECLARE_ANIMEVENT(AE_DEADEYE_SWATITEM)
DECLARE_ANIMEVENT(AE_DEADEYE_STARTSWAT)
DECLARE_ANIMEVENT(AE_DEADEYE_STEP_LEFT)
DECLARE_ANIMEVENT(AE_DEADEYE_STEP_RIGHT)
DECLARE_ANIMEVENT(AE_DEADEYE_SCUFF_LEFT)
DECLARE_ANIMEVENT(AE_DEADEYE_SCUFF_RIGHT)
DECLARE_ANIMEVENT(AE_DEADEYE_ATTACK_SCREAM)
DECLARE_ANIMEVENT(AE_DEADEYE_GET_UP)
DECLARE_ANIMEVENT(AE_DEADEYE_POUND)
DECLARE_ANIMEVENT(AE_DEADEYE_ALERTSOUND)
DECLARE_ANIMEVENT(AE_DEADEYE_POPHEADCRAB)

DECLARE_INTERACTION(g_interactionDeadEyeMeleeWarning)

DEFINE_SCHEDULE
(
SCHED_DEADEYE_MOVE_SWATITEM,

"	Tasks"
"		TASK_DEADEYE_DELAY_SWAT			3"
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_CHASE_ENEMY"
"		TASK_DEADEYE_GET_PATH_TO_PHYSOBJ	0"
"		TASK_WALK_PATH					0"
"		TASK_WAIT_FOR_MOVEMENT			0"
"		TASK_FACE_ENEMY					0"
"		TASK_DEADEYE_SWAT_ITEM			0"
"	"
"	Interrupts"
...
DEFINE_SCHEDULE
(
SCHED_DEADEYE_RELEASECRAB,

"	Tasks"
"		TASK_PLAY_PRIVATE_SEQUENCE_FACE_ENEMY		ACTIVITY:ACT_DEADEYE_RELEASECRAB"
"		TASK_DEADEYE_RELEASE_HEADCRAB				0"
"		TASK_DEADEYE_DIE								0"
"	"
"	Interrupts"
"		COND_TASK_FAILED"
...
"		TASK_STOP_MOVING				0"
"		TASK_TURN_LEFT					180"
"		TASK_SET_SCHEDULE				SCHEDULE:SCHED_DEADEYE_WAIT_AMBUSH"
"	"
"	Interrupts"
"		COND_TASK_FAILED"
"		COND_NEW_ENEMY"
...
"		TASK_SET_SCHEDULE				SCHEDULE:SCHED_DEADEYE_WANDER_MEDIUM" // keep doing it
"	"
"	Interrupts"
"		COND_NEW_ENEMY"
"		COND_SEE_ENEMY"
...
DEFINE_SCHEDULE
(
SCHED_DEADEYE_WANDER_FAIL,

"	Tasks"
"		TASK_STOP_MOVING		0"
"		TASK_WAIT				1"
"		TASK_SET_SCHEDULE		SCHEDULE:SCHED_DEADEYE_WANDER_MEDIUM"
"	Interrupts"
"		COND_NEW_ENEMY"
"		COND_LIGHT_DAMAGE"
"		COND_HEAVY_DAMAGE"
...
"		TASK_MELEE_ATTACK1		0"
"		TASK_SET_SCHEDULE		SCHEDULE:SCHED_DEADEYE_POST_MELEE_WAIT"
""
"	Interrupts"
"		COND_LIGHT_DAMAGE"
"		COND_HEAVY_DAMAGE"
...
//=========================================================
// Make the deadeye wait a frame after a melee attack, to
// allow itself & it's enemy to test for dynamic scripted sequences.
//=========================================================
DEFINE_SCHEDULE
(
SCHED_DEADEYE_POST_MELEE_WAIT,

"	Tasks"
"		TASK_DEADEYE_WAIT_POST_MELEE		0"
)
