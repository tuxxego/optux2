...
ConVar	sk_ShadowSoldier_s_health( "sk_ShadowSoldier_s_health","130");
ConVar	sk_ShadowSoldier_s_kick( "sk_ShadowSoldier_s_kick","15");

ConVar sk_ShadowSoldier_guard_health( "sk_ShadowSoldier_guard_health", "0");
ConVar sk_ShadowSoldier_guard_kick( "sk_ShadowSoldier_guard_kick", "0");
 
// Whether or not the combine guard should spawn health on death
ConVar sk_ShadowSoldier_die_spawn_health( "sk_ShadowSoldier_die_spawn_health", "1" );

extern ConVar sk_plr_dmg_buckshot;	
extern ConVar sk_plr_num_shotgun_pellets;

//Whether or not the combine should spawn health on death
ConVar	ShadowSoldier_spawn_health( "ShadowSoldier_spawn_health", "1" );

LINK_ENTITY_TO_CLASS(npc_combine_shadowsoldier, CNPC_ShadowSoldier);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ShadowSoldier::Spawn( void )
{
	Precache();
	...
	{
		// Stronger, tougher.
		SetHealth( sk_ShadowSoldier_guard_health.GetFloat() );
		SetMaxHealth( sk_ShadowSoldier_guard_health.GetFloat() );
		SetKickDamage( sk_ShadowSoldier_guard_kick.GetFloat() );
	}
	else
	{
		SetHealth( sk_ShadowSoldier_s_health.GetFloat() );
		SetMaxHealth( sk_ShadowSoldier_s_health.GetFloat() );
		SetKickDamage( sk_ShadowSoldier_s_kick.GetFloat() );
	}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ShadowSoldier::Precache()
{
	...
	if( !GetModelName() )
	{
		SetModelName( MAKE_STRING( "models/ShadowSoldier/combine_soldier_prisonguard.mdl" ) );
	}
...
void CNPC_ShadowSoldier::DeathSound( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
void CNPC_ShadowSoldier::ClearAttackConditions( )
{
...
void CNPC_ShadowSoldier::PrescheduleThink( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_ShadowSoldier::BuildScheduleTestBits( void )
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_ShadowSoldier::SelectSchedule ( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_ShadowSoldier::GetHitgroupDamageMultiplier( int iHitGroup, const CTakeDamageInfo &info )
{
	switch( iHitGroup )
	{
	case HITGROUP_HEAD:
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_ShadowSoldier::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
...
void CNPC_ShadowSoldier::OnChangeActivity( Activity eNewActivity )
{
...
void CNPC_ShadowSoldier::OnListened()
{
...
//-----------------------------------------------------------------------------
void CNPC_ShadowSoldier::Event_Killed( const CTakeDamageInfo &info )
{
	// Don't bother if we've been told not to, or the player has a megaphyscannon
	if ( ShadowSoldier_spawn_health.GetBool() == false || PlayerHasMegaPhysCannon() )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_ShadowSoldier::IsLightDamage( const CTakeDamageInfo &info )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_ShadowSoldier::IsHeavyDamage( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
Activity CNPC_ShadowSoldier::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_ShadowSoldier )
