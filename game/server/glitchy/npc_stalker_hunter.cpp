...
ConVar	sk_StalkerHunter_s_health( "sk_StalkerHunter_s_health","130");
ConVar	sk_StalkerHunter_s_kick( "sk_StalkerHunter_s_kick","15");

ConVar sk_StalkerHunter_guard_health( "sk_StalkerHunter_guard_health", "0");
ConVar sk_StalkerHunter_guard_kick( "sk_StalkerHunter_guard_kick", "0");
 
// Whether or not the combine guard should spawn health on death
ConVar sk_StalkerHunter_die_spawn_health( "sk_StalkerHunter_die_spawn_health", "1" );

extern ConVar sk_plr_dmg_buckshot;	
extern ConVar sk_plr_num_shotgun_pellets;

//Whether or not the combine should spawn health on death
ConVar	StalkerHunter_spawn_health( "StalkerHunter_spawn_health", "1" );

LINK_ENTITY_TO_CLASS(npc_stalker_hunter, CNPC_StalkerHunter);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_StalkerHunter::Spawn( void )
{
	Precache();
	...
	{
		// Stronger, tougher.
		SetHealth( sk_StalkerHunter_guard_health.GetFloat() );
		SetMaxHealth( sk_StalkerHunter_guard_health.GetFloat() );
		SetKickDamage( sk_StalkerHunter_guard_kick.GetFloat() );
	}
	else
	{
		SetHealth( sk_StalkerHunter_s_health.GetFloat() );
		SetMaxHealth( sk_StalkerHunter_s_health.GetFloat() );
		SetKickDamage( sk_StalkerHunter_s_kick.GetFloat() );
	}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_StalkerHunter::Precache()
{
	...
	if( !GetModelName() )
	{
		SetModelName( MAKE_STRING( "models/killzone/Combine_Soldier.mdl" ) );
	}
...
void CNPC_StalkerHunter::DeathSound( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
void CNPC_StalkerHunter::ClearAttackConditions( )
{
...
void CNPC_StalkerHunter::PrescheduleThink( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_StalkerHunter::BuildScheduleTestBits( void )
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_StalkerHunter::SelectSchedule ( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_StalkerHunter::GetHitgroupDamageMultiplier( int iHitGroup, const CTakeDamageInfo &info )
{
	switch( iHitGroup )
	{
	case HITGROUP_HEAD:
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_StalkerHunter::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
...
void CNPC_StalkerHunter::OnChangeActivity( Activity eNewActivity )
{
...
void CNPC_StalkerHunter::OnListened()
{
...
//-----------------------------------------------------------------------------
void CNPC_StalkerHunter::Event_Killed( const CTakeDamageInfo &info )
{
	// Don't bother if we've been told not to, or the player has a megaphyscannon
	if ( StalkerHunter_spawn_health.GetBool() == false || PlayerHasMegaPhysCannon() )
	{
...
#ifdef MAPBASE
				CBaseEntity *pItem;
				if (GetActiveWeapon() && FClassnameIs(GetActiveWeapon(), "weapon_oicw"))
...
Class_T	CNPC_StalkerHunter::Classify()
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_StalkerHunter::IsLightDamage( const CTakeDamageInfo &info )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_StalkerHunter::IsHeavyDamage( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
Activity CNPC_StalkerHunter::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_StalkerHunter )
