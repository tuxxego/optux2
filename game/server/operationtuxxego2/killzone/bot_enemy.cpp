...
ConVar	sk_BotEnemy_s_health( "sk_BotEnemy_s_health","130");
ConVar	sk_BotEnemy_s_kick( "sk_BotEnemy_s_kick","15");

ConVar sk_BotEnemy_guard_health( "sk_BotEnemy_guard_health", "0");
ConVar sk_BotEnemy_guard_kick( "sk_BotEnemy_guard_kick", "0");
 
// Whether or not the combine guard should spawn health on death
ConVar sk_BotEnemy_die_spawn_health( "sk_BotEnemy_die_spawn_health", "1" );

extern ConVar sk_plr_dmg_buckshot;	
extern ConVar sk_plr_num_shotgun_pellets;

//Whether or not the combine should spawn health on death
ConVar	BotEnemy_spawn_health( "BotEnemy_spawn_health", "1" );

LINK_ENTITY_TO_CLASS(bot_enemy, CNPC_BotEnemy);
...
void CNPC_BotEnemy::Spawn( void )
{
	...
	{
		// Stronger, tougher.
		SetHealth( sk_BotEnemy_guard_health.GetFloat() );
		SetMaxHealth( sk_BotEnemy_guard_health.GetFloat() );
		SetKickDamage( sk_BotEnemy_guard_kick.GetFloat() );
	}
	else
	{
		SetHealth( sk_BotEnemy_s_health.GetFloat() );
		SetMaxHealth( sk_BotEnemy_s_health.GetFloat() );
		SetKickDamage( sk_BotEnemy_s_kick.GetFloat() );
	}
...
Class_T	CNPC_BotEnemy::Classify()
{
...
void CNPC_BotEnemy::DeathSound( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
void CNPC_BotEnemy::ClearAttackConditions( )
{
...
void CNPC_BotEnemy::PrescheduleThink( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_BotEnemy::BuildScheduleTestBits( void )
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_BotEnemy::SelectSchedule ( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_BotEnemy::GetHitgroupDamageMultiplier( int iHitGroup, const CTakeDamageInfo &info )
{
	switch( iHitGroup )
	{
	case HITGROUP_HEAD:
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_BotEnemy::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
...
void CNPC_BotEnemy::OnChangeActivity( Activity eNewActivity )
{
...
//-----------------------------------------------------------------------------
void CNPC_BotEnemy::Event_Killed( const CTakeDamageInfo &info )
{
	// Don't bother if we've been told not to, or the player has a megaphyscannon
	if ( BotEnemy_spawn_health.GetBool() == false || PlayerHasMegaPhysCannon() )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_BotEnemy::IsLightDamage( const CTakeDamageInfo &info )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_BotEnemy::IsHeavyDamage( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
Activity CNPC_BotEnemy::NPC_TranslateActivity( Activity eNewActivity )
{
